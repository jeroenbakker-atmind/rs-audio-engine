...
// My try to fix the nemus processor, just keep for reference as I want to go back to the basics
#[derive(Default, Debug)]
pub struct NemusTryProcessor {
    pub string: String,
    ...
    /// Bow Pressure
    pub fb: f64,
    /// Bow Velocity in meters per second.
    pub vb: f64,
    // TODO: Make a const
    a: f64,
...
impl StringProcessor for NemusTryProcessor {
    fn new(sample_rate: f64, string: &String) -> NemusTryProcessor {
        let mut processor = NemusTryProcessor::default();
    ...
    fn compute_state(&mut self) {
        ...
        let zeta_1 = (0..self.modes_number as usize)
            .map(|mode_number| {
                self.modes_in[mode_number] * self.states[mode_number]
                    + self.modes_in[mode_number]
                        * self.states[mode_number + self.modes_number as usize]
            })
            .sum::<f64>();
        ...
        (0..self.modes_number as usize).for_each(|mode_number| {
            let zeta_2 = self.modes_in[mode_number] * zeta_1;
            let b1 = self.b11[mode_number] * self.states[mode_number]
                + self.b12[mode_number] * self.states[mode_number + self.modes_number as usize];
            let b2 = self.b21[mode_number] * self.states[mode_number]
                + self.b22[mode_number] * self.states[mode_number + self.modes_number as usize]
                + zeta_2 * 0.5 * self.timestep * self.fb * (lambda - 2.0 * d)
                + self.timestep * self.fb * d * self.modes_in[mode_number] * self.vb;
            let z1 = 0.5 * self.timestep * self.fb * lambda * self.modes_in[mode_number];
            ...
            vt1 += self.modes_in[mode_number] * self.inv_av2[mode_number];
            vt2 += self.modes_in[mode_number] * self.inv_ab2[mode_number];
        });

        let coeff = 1.0 / (1.0 + vt1);

        (0..self.modes_number as usize).for_each(|mode_number| {
            self.states[mode_number] =
    ...
    fn read_output(&mut self) -> f64 {
        let result = if self.is_being_played {
            (0..self.modes_number as usize)
                .map(|mode_number| self.modes_in[mode_number] * self.states[mode_number])
                .sum::<f64>()
        } else {
            0.0
        };
...
impl NemusTryProcessor {
    ...
    fn initialize_in_modes(&mut self) {
        self.modes_in = vec![0.0; self.modes_number as usize];
        self.recompute_in_modes();
    }

    fn recompute_in_modes(&mut self) {
        (0..self.modes_number as usize).for_each(|mode_number| {
            self.modes_in[mode_number] = self.compute_mode(self.excit_position, mode_number + 1);
        });
    }
    ...
    fn initialize_states(&mut self) {
        self.states = vec![0.0; self.modes_number as usize * 2];
    }
