...
impl Parameters {
    ...
    pub fn semitone(&self, step: usize) -> f32 {
        match self.step_type {
            ...
            StepType::FrequencyRange(_params) => self.steps as f32 / self.period_duration(step),
        }
    }
    ...
    pub fn frequency(&self, step: usize) -> f32 {
        match self.step_type {
            ...
            StepType::FrequencyRange(range) => {
                ...
                let frequency =
                    range.end_frequency * step_f32 + range.start_frequency * (1.0 - step_f32);
                frequency
            }
        }
    }
    ...
    pub fn period_duration(&self, step: usize) -> f32 {
        match self.step_type {
            StepType::Semitones => 1.0 / (step as f32 + 1.0),
            StepType::FrequencyRange(range) => {
