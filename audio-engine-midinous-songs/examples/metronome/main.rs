use audio_engine_midinous::{player::Player, song::Song};
use audio_engine_midinous_songs::SongLibrary;
...
fn main() -> Result<(), ()> {
    ...
    let song = SongLibrary::Metronome.create();
...
fn play_song(device: &cpal::Device, config: &cpal::StreamConfig, song: Song) -> Result<(), ()> {
    ...
    let samples = player.render(10.0);
